...
- **Types**: `char`/`signed char`/`unsigned char` as distinct 1B types, `short` (2B), `int` (4B), `long`/`long long` (8B with distinct C ranks), `float` (4B), `double` (8B), `void`, pointers, arrays, `struct`, `union`, tagged `enum`, `typedef`, `signed`/`unsigned` integer type specifiers, typed integer literal suffixes (`U`, `L`, `UL`, `LL`, `ULL`), semantic `const`/`volatile` qualifiers (including pointer qualifiers and qualifier-safe pointer conversions); enumerators accept integer constant expressions
