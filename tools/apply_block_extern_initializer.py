...
if Path("test/block_extern_initializer.sh").exists():
    print("block extern initializer migration already applied")
    raise SystemExit(0)
...
replace_once(
    "README.md",
    'rejection of direct `void` objects and void/function-typed record members, block-scope `auto` objects',
    'rejection of direct `void` objects and void/function-typed record members, rejection of block-scope `extern` initializers, block-scope `auto` objects',
)
...
Path("test/block_extern_initializer.sh").write_text(r'''#!/bin/bash
set -eu
...
print("block extern initializer migration applied")
