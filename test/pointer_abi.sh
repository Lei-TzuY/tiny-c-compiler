...
int host_apply(int (*fn)(int), int x) { return fn(x); }
int host_seventh_callback(int a, int b, int c, int d, int e, int f,
                          int (*fn)(int), int x) {
    (void)a; (void)b; (void)c; (void)d; (void)e; (void)f;
    return fn(x);
}
...
int host_apply(int (*)(int), int);
int host_seventh_callback(int, int, int, int, int, int, int (*)(int), int);

int local_plus3(int x) { return x + 3; }
...
int main(void) {
    ...
    if (host_apply(local_plus3, 39) != 42) return 9;
    if (host_seventh_callback(1, 2, 3, 4, 5, 6, local_plus3, 39) != 42) return 10;
    return 0;
}
EOF
...
# according to the platform ABI. Also exercise function-pointer returns and
# function-pointer parameters so recursive declarators stay interoperable at
# real ABI edges in both register and stack argument positions.
...
int minicc_apply(int (*fn)(int), int x) { return fn(x); }
int minicc_seventh_callback(int a, int b, int c, int d, int e, int f,
                            int (*fn)(int), int x) {
    return fn(x);
}
EOF
...
int minicc_apply(int (*)(int), int);
int minicc_seventh_callback(int, int, int, int, int, int, int (*)(int), int);

static int host_plus4(int x) { return x + 4; }
...
int main(void) {
    ...
    if (minicc_apply(host_plus4, 38) != 42) return 10;
    if (minicc_seventh_callback(1, 2, 3, 4, 5, 6, host_plus4, 38) != 42) return 11;
    return 0;
}
EOF
